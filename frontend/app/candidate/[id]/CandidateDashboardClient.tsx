...
export default function CandidateDashboardClient({ id: propId }: Props) {
  const params = useParams();
  const rawId = propId || (params?.id as string) || '';
  const [candidateId, setCandidateId] = useState<string>(rawId);
    ...
    return (
      <div className="max-w-2xl mx-auto my-16 p-8 bg-white border border-slate-200 rounded-xl text-center space-y-4 shadow-xs">
        ...
        <p className="text-xs text-slate-600">
          We could not locate an official public record for <code className="bg-slate-100 px-1 py-0.5 rounded">{candidateId || 'this candidate'}</code>.
        </p>
        <a
          href="/"
